...
import { Cpu, Scale, Maximize2, Orbit, Sparkles, Clock, Globe } from 'lucide-react';
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {facts.map((fact, idx) => {
          ...
          return (
            <Card key={idx} className="border-slate-200 bg-white/80 hover:border-slate-300 dark:border-slate-800/80 dark:bg-slate-950/60 dark:hover:border-slate-700 transition-all">
              <div className="flex items-start gap-3">
                <div className={`mt-0.5 rounded-lg bg-slate-100 dark:bg-slate-900/80 p-2.5 border border-slate-200 dark:border-slate-800 ${fact.color}`}>
                  <Icon className="h-5 w-5" />
                </div>
                <div>
                  <div className="text-xs font-semibold text-slate-500 dark:text-slate-400 font-mono uppercase tracking-wider">
                    {fact.label}
                  </div>
